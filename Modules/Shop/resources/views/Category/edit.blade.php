    ...
    <div class="main-content">
        <section class="section">
            <x-admin.breadcrumb title="{{ __('Edit Category') }}" :list="[
                ...
                __('Category List') => route('admin.product.category.index'),
                __('Edit Category') => '#',
            ]" />
                                ...
                                <ul class="lang_list">
                                    @foreach ($languages as $language)
                                        <li><a id="{{ request('code') == $language->code ? 'selected-language' : '' }}"
                                                href="{{ route('admin.product.category.edit', ['category' => $category->id, 'code' => $language->code]) }}"><i
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                ...
                                <div>
                                    <x-admin.back-button :href="route('admin.product.category.index')" />
                                </div>
                            </div>
                            <div class="card-body">
                                <form
                                    action="{{ route('admin.product.category.update', ['category' => $category->id, 'code' => $code]) }}"
                                    method="post">
                                    @csrf
                                    @method('PUT')
                                    <div class="row">
