...
@section('title')
    <title>{{ __('Project Section') }}</title>
@endsection
@php
...
@section('admin-content')
    <div class="main-content">
        <section class="section">
            <x-admin.breadcrumb title="{{ __('Project Section') }} ( {{$current_theme_title}} )" :list="[
                                ...
                                <ul class="lang_list">
                                    @foreach ($languages as $language)
                                        <li><a id="{{ request('code') == $language->code ? 'selected-language' : '' }}"
                                                href="{{ route('admin.project-section.index', ['code' => $language->code]) }}"><i
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                <x-admin.form-title :text="__('Project Section')" />
                                <div>
                                    <x-admin.back-button :href="route('admin.dashboard')" />
                                </div>
                            </div>
                            <div class="card-body">
                                <form action="{{ route('admin.project-section.update', ['code' => $code]) }}"
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <x-admin.form-input id="title" data-translate="true" name="title"
                                                    ...
                                                    value="{{ $projectSection?->getTranslation($code)?->content?->title }}"
                                                    required="true" />
                                            </div>
                                        </div>
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <x-admin.form-input id="sub_title" data-translate="true" name="sub_title"
                                                    ...
                                                    value="{{ $projectSection?->getTranslation($code)?->content?->sub_title }}"
                                                    required="false" />
                                            </div>
                                        </div>
                                        <div class="col-md-12">
